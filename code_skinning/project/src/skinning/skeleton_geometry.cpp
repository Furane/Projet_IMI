...
skeleton_geometry local_to_global(skeleton_geometry const& sk_local,skeleton_parent_id const& parent_id)
{
    ...
    //TO Do: Completez la structure sk_global en exprimant les reperes dans les coordonnees globales.
    {
        for(int i=0; i<N; ++i)
        {
            //si le joint n'a pas de parent
            if(parent_id[i] == -1)
                sk_global.push_back(skeleton_joint(sk_local[i].position, sk_local[i].orientation));

            else
            {
                cpe::quaternion q = sk_global[parent_id[i]].orientation * sk_local[i].orientation;
                cpe::vec3 p = sk_global[parent_id[i]].orientation * sk_local[i].position+sk_global[parent_id[i]].position;
                sk_global.push_back(skeleton_joint(p,q));
            }
        }
    }
    return sk_global;
...
skeleton_geometry inversed(skeleton_geometry const& skeleton)
{
    ...
    for(skeleton_joint const& joint : skeleton)
    {
        ...
        vec3 p = -joint.position;
        sk_inversed.push_back(skeleton_joint(p,q));
...
std::vector<vec3> extract_bones(skeleton_geometry const& skeleton,skeleton_parent_id const& parent_id)
{
    ...
    for(int k=0 ; k<N_joint ; ++k)//int k=1
    {
